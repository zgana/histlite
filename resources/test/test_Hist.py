...
class TestHist (unittest.TestCase):
    ...
    def test_err (self):
        ...
        self.assertTrue (
            eps > np.abs (h1.get_errors ([.25]) - np.sqrt (2))
        )
        self.assertTrue (
            1e-6 > np.max (h1.errors - np.sqrt (2))
        )
        self.assertTrue (
            1e-6 > np.max (h2.errors - np.sqrt (3))
        )
        hadd = h1 + h2
        self.assertTrue (
            1e-6 > np.max (hadd.errors - np.sqrt (2 + 3))
        )
        hadd = 1 + h1
        self.assertTrue (
            1e-6 > np.max (hadd.errors - np.sqrt (2))
        )
        hsub = h2 - h1
        self.assertTrue (
            1e-6 > np.max (hsub.errors - np.sqrt (2 + 3))
        )
        hsub = 1 - h1
        self.assertTrue (
            1e-6 > np.max (hsub.errors - np.sqrt (2))
        )
        hmul = h1 * h2
        self.assertTrue (
            1e-6 > np.max (
                hmul.errors - 6 * np.sqrt (1/np.sqrt (2) + 1/np.sqrt (3)))
        )
        hmul = 2 * h1
        self.assertTrue (
            1e-6 > np.max (
                hmul.errors - 2 * np.sqrt (2))
        )
        hdiv = h2 / h1
        self.assertTrue (
            1e-6 > np.max (
                hdiv.errors - 1.5 * np.sqrt (1/np.sqrt (2) + 1/np.sqrt (3)))
        )
        hdiv = h1 / 2.
        self.assertTrue (
            1e-6 > np.max (
                hdiv.errors - 0.5 * np.sqrt (2))
        )
